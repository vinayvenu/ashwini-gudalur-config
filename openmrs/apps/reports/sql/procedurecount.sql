...
else 'Tribe' END) as Caste,pc.name,sol.name as "Product Name",count(sol.id),sol.price_unit,round(sum(sol.product_uom_qty*sol.price_unit),2) as Total from sale_order_line sol
