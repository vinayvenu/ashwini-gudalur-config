select p.person_id,CONCAT(pn.given_name, " ", pn.family_name) as Name.city_village,padd.address2,v.visit_id,e.encounter_id,GROUP_CONCAT(DISTINCT(cdvn.name))as Diagnosis,GROUP_CONCAT(DISTINCT(obs.value_text))as 'ADT Notes' from  confirmed_diagnosis_view_new cdvn
