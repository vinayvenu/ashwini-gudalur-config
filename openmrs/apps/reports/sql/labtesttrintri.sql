...
from orders ord join obs o on ord.order_id = o.order_id and order_type_id=3
  ...
  join person_attribute pa on p.person_id = pa.person_id and pa.person_attribute_type_id=27 AND pa.value=2  where cast(o.date_created as DATE) between '#startDate#' and '#endDate#') as 'Non Tribal',
...
from orders ord join obs o on ord.order_id = o.order_id and order_type_id=3
  ...
  join visit v on v.visit_id=en.visit_id
  join person p on v.patient_id=p.person_id
  join person_attribute pa on p.person_id = pa.person_id and pa.person_attribute_type_id=27 AND pa.value=1 where cast(o.date_created as DATE) between '#startDate#' and '#endDate#') as 'Tribal';